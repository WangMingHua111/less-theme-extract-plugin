...
function pitch(request) {
    ...
    var asyncs = Object.keys(themes).map(theme => {
        return new Promise(resolve => {
            const lessOptions = themes[theme]
            const loaders = this.loaders.slice(this.loaderIndex + 2);

            const childCompiler = this._compilation.createChildCompiler(`${pluginName} ${theme} ${request}`, outputOptions);
            ...
            childCompiler.hooks.thisCompilation.tap(`${pluginName} ${theme} loader`, compilation => {
                compilation.hooks.normalModuleLoader.tap(`${pluginName} ${theme} loader`, (loaderContext, module) => {
                    ...
                    if (module.request === request) {
                        ...
                        module.loaders = loaders.map(loader => {
                            let options = /less-loader/i.test(loader.path) ? { ...loader.options, lessOptions } : loader.options
                            return {
                                loader: loader.path,
                                options: options,
                                ident: loader.ident
            ...
            childCompiler.runAsChild((err, entries, compilation) => {
                ...
                if (compilation.errors.length > 0) {
                    return resolve(compilation.errors[0]);
                }
                ...
                if (!source) {
                    return resolve(new Error("Didn't get a result from child compiler"));
                }
                ...
                return resolve(null, resultSource);
            });
        })
    })
    Promise.all(asyncs).then(req => {
        callback(null, `// extracted by ${pluginName}`)
    })
